    ...
    public static class AzureAdB2CAuthenticationBuilderExtensions
    {
        ...
        private class ConfigureAzureOptions : IConfigureNamedOptions<OpenIdConnectOptions>
        {
            ...
            public void Configure(string name, OpenIdConnectOptions options)
            {
                ...
                options.Authority =
                    $"{this._azureOptions.Instance}/{this._azureOptions.Domain}/{this._azureOptions.SignUpSignInPolicyId}/v2.0";
                ...
                options.Events = new OpenIdConnectEvents
                                     {
                                         OnRedirectToIdentityProvider =
                                             this.OnRedirectToIdentityProvider,
            ...
            {
                var defaultPolicy = this._azureOptions.DefaultPolicy;
                if (context.Properties.Items.TryGetValue(AzureAdB2COptions.PolicyAuthenticationProperty, out var policy)
                    && !policy.Equals(defaultPolicy))
                {
                    context.ProtocolMessage.Scope = OpenIdConnectScope.OpenIdProfile;
                    context.ProtocolMessage.ResponseType = OpenIdConnectResponseType.IdToken;
                    context.ProtocolMessage.IssuerAddress = context.ProtocolMessage.IssuerAddress.ToLower().Replace(
                        $"/{defaultPolicy.ToLower()}/",
                        $"/{policy.ToLower()}/");
